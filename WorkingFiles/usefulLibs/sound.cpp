...
// Sets file based a user specified filename
void sound::setInput(char *pC){
 free(in);
 free(file);
...
FILE * sound::getFile(){
 if(flag == 1){
  fprintf(stderr, "Warning this action changes class type!\n");
...
char * sound::getName(){
 if(flag != 1){
  fprintf(stderr, "This action is not supported in this class type!");
  return NULL;
 }
